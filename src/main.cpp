    ...
    do
    {
        ...
        for (size_t i = 0; i < 5; ++i) // считывание слова по буквам из консоли
        {
            ...
            if (vec[i] == '\n') // если какой-то из элментов вектора заполнился '\n', значит слово короткое
            {
                ...
                std::cout << "\x1b[4F\x1b[0J"; // стираем надписи предупреждения и введенное слово
                flag = true;
                break;
            }
        }
        ...
        if (vec[4] != '\0' && vec[4] != '\n')
        {
            ...
            if ((vec[5] != '\n') && flag == false) // если  пользователь ввел больше 5 букв, должен перепечатать
            {
                ...
                std::cout << "\x1b[4F\x1b[0J"; // очистка консоли от длинного слова и предупреждения
                flag = true;
            }
        }
...
int main(){
	...
	std::vector<char> user_word(++n); // вектор всегда будет больше 
